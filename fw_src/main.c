...
static int16_t TMP1075_ReadCelsius_x10(void)
{
    double tC = TMP1075_ReadCelsius();
    return (int16_t)(tC * 10.0);
}
    ...
    while(1){
        if (PIR0bits.TMR0IF){
            ...
            if (ms_counter >= 5000)
            {
                ...
                int16_t tempCx10       = TMP1075_ReadCelsius_x10();
                int16_t tempC_int      = tempCx10 / 10;
                int16_t tempC_frac     = tempCx10 % 10;
                if (tempC_frac < 0) { tempC_frac = -tempC_frac; }

                printf("C1=%u mV  C2=%u mV  C3=%u mV  C4=%u mV  I=%d mA  T=%d.%d C\r\n",
                        cellVoltages.c1_mv,
                        cellVoltages.c2_mv,
                        cellVoltages.c3_mv,
                        cellVoltages.c4_mv,
